...
void PlotTrkChi2(int sample = SAMPLE) {
...
void plot_trk_chi2() {
  ...
  TH1F* h_ref = f_ref->Get<TH1F>("trk_chi2");
  ...
  for(int i = 0; i < N_CASES.size(); i++) {
    ...
    for(auto& s : cases) {
      ...
      TH1F* h = f->Get<TH1F>("trk_chi2");
      h->SetMarkerSize(0.0);
      h->SetLineColor(COLOR[s]);
      h->GetXaxis()->SetRangeUser(0.0, 30.0);
      h->SetMaximum(TRK_CHI2_MAX);
      h->GetXaxis()->SetTitle("L1 Track #chi^{2}");
    ...
    f_name = "../plots/" + N_SAMPLE + "_trk_chi2_" + N_CASES[i] + ".pdf";
  ...
  if(DO_DETAILED_PLOTS) {
    ...
    h_ref->GetXaxis()->SetRangeUser(0.0, 30.0);
    h_ref->SetMaximum(TRK_CHI2_MAX);
    h_ref->GetXaxis()->SetTitle("L1 Track #chi^{2}");
    ...
    for(int s = 1; s < F_FAILS.size(); s++) {
      ...
      f_name = "../plots/" + N_SAMPLE + "_trk_chi2_" + N_SAVE + std::to_string(s) + ".pdf";
      c.SaveAs(f_name.c_str());
