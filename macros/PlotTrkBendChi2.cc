...
void plot_trk_bendchi2();


void PlotTrkBendChi2(int sample = SAMPLE) {
...
void plot_trk_bendchi2() {
  ...
  TH1F* h_ref = f_ref->Get<TH1F>("trk_bendchi2");
  ...
  for(int i = 0; i < N_CASES.size(); i++) {
    ...
    for(auto& s : cases) {
      ...
      TH1F* h = f->Get<TH1F>("trk_bendchi2");
      h->SetMarkerSize(0.0);
      h->SetLineColor(COLOR[s]);
      h->GetXaxis()->SetRangeUser(0.0, 2.2);
      h->SetMaximum(TRK_BENDCHI2_MAX);
      h->GetXaxis()->SetTitle("L1 Track #chi^{2}_{bend}");
    ...
    f_name = "../plots/" + N_SAMPLE + "_trk_bendchi2_" + N_CASES[i] + ".pdf";
  ...
  if(DO_DETAILED_PLOTS) {
    ...
    h_ref->SetMaximum(TRK_BENDCHI2_MAX);
    h_ref->GetXaxis()->SetTitle("L1 Track #chi^{2}_{bend}");
    ...
    for(int s = 1; s < F_FAILS.size(); s++) {
      ...
      f_name = "../plots/" + N_SAMPLE + "_trk_bendchi2_" + N_SAVE + std::to_string(s) + ".pdf";
      c.SaveAs(f_name.c_str());
