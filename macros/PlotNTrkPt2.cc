...
void plot_ntrk_pt2() {
  ...
  for(int i = 0; i < N_CASES.size(); i++) {
    ...
    TLegend* l = new TLegend(NTRK_PT2_LX1, 0.72, NTRK_PT2_LX2, 0.93);
    l->SetFillStyle(0);
    l->SetBorderSize(0);
    l->SetTextSize(0.04);
    l->SetTextFont(42);

    TLegend* l_mean = new TLegend(NTRK_PT2_LMEANX1, 0.45, NTRK_PT2_LMEANX2, 0.7);
    ...
    if(NTRK_PT2_MAX_ZOOM > 0.0) {
      h_ref->GetXaxis()->SetRangeUser(0.0, NTRK_PT2_MAX_ZOOM);
